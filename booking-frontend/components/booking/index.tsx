      ...
      <div className="mx-auto flex max-w-lg flex-col items-center gap-3 px-4">
        ...
        >
          <img
            ...
            alt="Club Amsterdam"
            className="h-20 w-20 animate-in fade-in zoom-in-75 rounded-3xl border border-amber-500/20 bg-black/50 p-3 duration-700 transition-colors group-hover:border-amber-500/50"
          />
        </button>
        ...
        <p className="animate-in fade-in slide-in-from-bottom-2 text-xs italic tracking-[0.2em] text-amber-500/60 duration-700 delay-300">
    ...
    <div className="flex items-center gap-2">
      {steps.map((label, index) => (
        <div key={label} className="flex items-center gap-2">
          ...
          {/* Active step label always visible; others hidden on small screens */}
          <span className={cn(
            "text-xs transition-all duration-300",
            index === step ? "block text-zinc-200 font-medium" : "hidden text-zinc-600 sm:block"
          )}>{label}</span>
          {index < steps.length - 1 && (
            <div className={cn(
              "mx-1 h-px w-5 shrink-0 transition-all duration-500",
        ...
        {date && slot ? (
          ...
          <div className="text-right">
            ...
            <p className="text-[10px] text-zinc-500">
              <Clock3 className="mr-0.5 inline h-2.5 w-2.5" />{selectedService.duration_minutes} min
            </p>
          </div>
        )}
      </div>
      ...
      <div className="flex min-h-screen items-center justify-center bg-zinc-950 px-6">
        <div className="w-full max-w-sm animate-in fade-in zoom-in-95 rounded-3xl border border-red-500/30 bg-zinc-900/90 p-8 text-center duration-500">
          ...
          <button
            ...
            className="mt-4 text-sm text-amber-500 underline hover:text-amber-400"
          >
            Reintentar
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-50">
      ...
      <main className="mx-auto max-w-lg px-4 py-6">
        ...
        >
          ...
          {step === 2 && (
            <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
              <CustomerForm />
              {submitError && (
                <p className="mt-3 rounded-xl bg-red-500/10 px-3 py-2 text-sm text-red-300">{submitError}</p>
              )}
            </div>
          )}
        </div>
        ...
        {/* Navigation + hint */}
        <div className="mt-4 space-y-2 animate-in fade-in slide-in-from-bottom-3 duration-500 delay-150">
          ...
          {/* Hint de qué falta */}
          {hint && !canContinue() && (
            <p className="animate-in fade-in text-center text-xs text-zinc-500 duration-200">
              {hint}
            </p>
          )}
        </div>

        {/* Price + payment reminder on step 2 */}
        {step === 2 && selectedService && (
          <p className="mt-3 animate-in fade-in text-center text-xs text-zinc-500 duration-500">
            {selectedService.name} · {selectedService.duration_minutes} min ·{" "}
            <span className="text-amber-500/70">{formatCurrency(selectedService.price_amount, selectedService.price_currency)}</span>
            {" "}— pago en el local
          </p>
        )}
      </main>
      ...
      <div className="rounded-3xl border border-zinc-800/60 bg-zinc-900/40 p-5 space-y-3">
        {business.address ? (
          <a href={business.maps_url || "#"} target="_blank" rel="noreferrer"
            ...
            <MapPin className="h-4 w-4 shrink-0 text-amber-500/60" />{business.address}
          </a>
        ) : null}
        {business.phone ? (
          <a href={`tel:${business.phone}`}
            className="flex items-center gap-3 text-sm text-zinc-400 transition-colors hover:text-amber-400">
            <Phone className="h-4 w-4 shrink-0 text-amber-500/60" />{business.phone}
          </a>
        ) : null}
        {business.instagram_url ? (
          <a href={business.instagram_url} target="_blank" rel="noreferrer"
            className="flex items-center gap-3 text-sm text-zinc-400 transition-colors hover:text-amber-400">
            <Instagram className="h-4 w-4 shrink-0 text-amber-500/60" />@clubamsterdam_
          </a>
        ) : null}
      </div>
    </footer>
